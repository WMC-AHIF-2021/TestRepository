...
class InsertHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "body":
            ...
            current_line = file_lines[line-1]
            after_lines = file_lines[line-1:]

            for pos, c in enumerate(current_line, col):
                if c == ">":
                    # Appending the banner in this exact location
                    current_line = f"{current_line[:pos+1]}" \
                                   f"\n{BANNER_TO_INSERT}\n" \
                                   f"{current_line[pos+1:]}"

            # Merging the strings back together
            self.content = f"{''.join(prev_lines)}" \
                           f"{''.join(current_line)}" \
                           f"{''.join(after_lines)}"
